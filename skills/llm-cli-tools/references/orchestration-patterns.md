...
run_model gemini gemini -p "$PROMPT" &
PID_G=$!
...
|------|-------------------|--------------------------|
| Gemini | File content appended as context ✅ (auto non-interactive when stdin is non-TTY) | File content becomes prompt ✅ (without `-p`) |
...
find src -name "*.py" -exec sh -c 'echo "=== {} ==="; cat {}' \; | gemini -p "Review:"
...
for f in src/**/*.py; do echo "=== $f ==="; cat "$f"; done | gemini -p "Review:"
```
...
if [ "$TOKENS" -gt 200000 ]; then
  ...
  gemini -p "Review this codebase for bugs and improvements:" < /tmp/src-bundle.txt
else
...
find src -name "*.py" -mtime -7 \
  -exec sh -c 'echo "=== {} ==="; cat {}' \; | gemini -p "Review recent changes:"
...
git diff main | gemini -p "Review this diff for bugs:"
git diff main | codex exec "Review this diff for bugs:"
...
gemini -p "Analyze this code:" < largefile.py

# Or use here-string for variables
gemini -p "Analyze:" <<< "$CONTENT"
```
...
ANALYSIS=$(gemini -p "Analyze architecture:" < design.md)
...
gemini -p "Review the architecture of this project"
...
gemini -p "Analyze:" < code.py | tee -a "$CONTEXT_FILE"
echo "---" >> "$CONTEXT_FILE"
...
retry_with_backoff gemini -p "prompt"
```
...
get_response() {
  ...
  # Fall back to Gemini
  gemini -p "$prompt" 2>/dev/null && return 0
...
timeout 30 gemini -p "prompt" || echo "Gemini timed out"

# Parallel with individual timeouts
timeout 30 gemini -p "prompt" > /tmp/g.txt &
...
gemini -p -o json "Answer this and rate your confidence 0-100:
$PROMPT
...
RESULT=$(gemini -p -o json "Rate confidence 0-100 and answer: $PROMPT" 2>/dev/null)
